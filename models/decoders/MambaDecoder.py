...
from models.encoders.vmamba import CVSSDecoderBlock, AlignedMambaBlock,MambaVisionBlock,AlignedMambaVisionBlock
...
class AlignedMambaVisionUpsample(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=nn.LayerNorm):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
        ...
        #     norm_layer(dim),
        # )
        self.aligned_mamba = AlignedMambaVisionBlock(
                hidden_dim= dim,
                mlp_ratio=0.0,
                d_state=4
                )
...
class MambaVision_up(nn.Module):
    def __init__(self, dim, input_resolution, depth, dt_rank="auto", 
                 d_state=4, ssm_ratio=2.0, attn_drop_rate=0., 
                 drop_rate=0.0, mlp_ratio=4.0,
                 drop_path=0.1, norm_layer=nn.LayerNorm, upsample=None, 
                 shared_ssm=False, softmax_version=False,num_heads=1,transformer_blocks=[],
                 use_checkpoint=False, **kwargs):
    ...
    def __init__(self, 
        ...
        for i_layer in range(self.num_layers):
            if i_layer == 0:
                ...
                # B, 60, 80, 192 -> B, 120, 160, 96
                # B, 120, 160, 96 -> B, 120, 160, 96
                layer_up = MambaVision_up(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                         input_resolution=(
                                         self.patches_resolution[0] // (2 ** (self.num_layers - 1 - i_layer)),
                                         self.patches_resolution[1] // (2 ** (self.num_layers - 1 - i_layer))),
                                         depth=depths[(self.num_layers - 1 - i_layer)],
