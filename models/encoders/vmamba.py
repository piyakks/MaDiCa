...
if True:
    ...
    def extract_non_zero_values(tensor):
        ...
        # Flatten and gather non-zero values
        non_zero_values = flat_tensor[mask]

        non_zero_values = non_zero_values.view(B, C, -1)
...
class AlignedMB_MambaVision(nn.Module):
    '''
    Multimodal Mamba Selective Scan 2D
    '''
    def __init__(
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        ...
        if self.d_conv > 1:
            x_rgb_trans = x_rgb.permute(0, 3, 1, 2).contiguous()
            x_e_trans = x_e.permute(0, 3, 1, 2).contiguous()
            x_rgb_trans_1d=rearrange(x_rgb_trans,' b c h w -> b c (h w)')
            x_e_trans_1d=rearrange(x_e_trans,' b c h w -> b c (h w)')
            
            # x_rgb_conv = self.act(self.conv2d(x_rgb_trans)) # (b, d, h, w)
            x_rgb_conv = F.silu(F.conv1d(input=x_rgb_trans_1d, weight=self.conv1d.weight, bias=self.conv1d.bias, padding='same', groups=self.d_inner))
            x_rgb_conv=rearrange(x_rgb_conv,' b c (h w) -> b c h w',h=h,w=w)
            
            # x_e_conv = self.act(self.conv1d_modalx(x_e_trans)) # (b, d, h, w)
            x_e_conv = F.silu(F.conv1d(input=x_e_trans_1d, weight=self.conv1d_modalx.weight, bias=self.conv1d_modalx.bias, padding='same', groups=self.d_inner))
            x_e_conv=rearrange(x_e_conv,' b c (h w) -> b c h w',h=2*h,w=2*w)
...
from mambavision.models.mamba_vision import Attention,MambaVisionMixer
...
class SaliencyMambaBlock(nn.Module):
    ...
    def _forward(self, x: torch.Tensor, gt: torch.Tensor):
        ...
        forward = self.drop_path(self.op(x, gt))
...
class SaliencyMambaVisionBlock(nn.Module):
    '''
    Saliency Mamba (SaMB), with 2d SSM
    '''
    def __init__(
        ...
        **kwargs,
        ...
        self.dim = hidden_dim
        # self.norm = norm_layer(hidden_dim)
        self.op = SaliencyMambaVision_SS2D(
            d_model=hidden_dim, 
            dropout=attn_drop_rate, 
            d_state=d_state, 
            ssm_ratio=ssm_ratio, 
    ...
    def forward(self, x: torch.Tensor, gt: torch.Tensor):
        ...
        if self.use_checkpoint:
            ...
            return self._forward(x, gt)
class SaliencyMambaBlock_scribble(nn.Module):
    '''
    Saliency Mamba (SaMB), with 2d SSM
    '''
    def __init__(
    ...
    def _forward(self, x: torch.Tensor, gt: torch.Tensor,back_gt: torch.Tensor):
        x = x + self.drop_path(self.op(x, gt))
        # y= x+self.drop_path(self.op2(x, back_gt))
        y= self.drop_path(self.op2(x, back_gt))
        x=x-y
        if self.mlp_branch:
            x = x + self.drop_path(self.mlp(self.norm2(x))) # FFN
    ...
    def forward(self, x: torch.Tensor, gt: torch.Tensor,back_gt: torch.Tensor):
        '''
        B C H W, B C H W -> B C H W
        '''
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, x, gt,back_gt)
        else:
            return self._forward(x, gt,back_gt)
...
class CAB(nn.Module):
    ...
    def forward(self, x):
        ...
        res += x
        return res       
class SaliencyMambaBlock_scribble_ca(nn.Module):
    '''
    Saliency Mamba (SaMB), with 2d SSM
    '''
    def __init__(
        ...
        **kwargs,
        ...
        self.drop_path = DropPath(drop_path)
        self.CAB = CAB(hidden_dim, kernel_size=3, reduction=4, bias=False, act=nn.PReLU())
        self.mlp_branch = mlp_ratio > 0
        if self.mlp_branch:
            self.norm2 = norm_layer(hidden_dim)
            mlp_hidden_dim = int(hidden_dim * mlp_ratio)
...
class AlignedMambaVisionBlock(nn.Module):
    '''
    Saliency Mamba (SaMB), with 2d SSM
    '''
    def __init__(
...
class Saliency_BG_MB_SS2D(nn.Module):
    '''
    Local Mamba Selective Scan 2D
    '''
    def __init__(
        ...
        **kwargs,
        ...
        self.x_proj_weight_back=nn.Parameter(torch.stack([t.weight for t in self.x_proj], dim=0)) # (K, N, inner)
        ...
        self.dt_projs_weight_back = nn.Parameter(torch.stack([t.weight for t in self.dt_projs], dim=0)) # (K, inner, rank)
        self.dt_projs_bias_back = nn.Parameter(torch.stack([t.bias for t in self.dt_projs], dim=0)) # (K, inner)
        del self.dt_projs
        ...
        self.A_logs_back = self.A_log_init(self.d_state, self.d_inner, copies=self.K2, merge=True) # (K * D, N)
        self.Ds_back = self.D_init(self.d_inner, copies=self.K2, merge=True) # (K * D)
        # out proj =======================================
        if not self.softmax_version:
            self.out_norm = nn.LayerNorm(self.d_inner)
        self.out_proj = nn.Linear(self.d_inner, self.d_model, bias=bias, **factory_kwargs)
    ...
    def forward_corev2_back(self, x: torch.Tensor, gt: torch.Tensor, nrows=-1):
        return cross_selective_scan_saliency_k2(
            x, gt, self.x_proj_weight_back, None, self.dt_projs_weight_back, self.dt_projs_bias_back,
            self.A_logs_back, self.Ds_back, getattr(self, "out_norm", None), self.softmax_version, 
            nrows=nrows, flip = self.flip
        )
    ...
    def forward(self, x: torch.Tensor, gt: torch.Tensor,back_gt: torch.Tensor):
        ...
        if self.d_conv > 1:
            ...
            x_conv = self.act(self.conv2d(x_trans)) # (b, d, h, w)
            y = self.forward_corev2(x_conv, gt) # b, d, h, w -> b, h, w, d
            back=self.forward_corev2_back(x_conv,back_gt)
            y=y-back
            # SE to get attention, scale
            b, d, h, w = x_trans.shape
            x_squeeze = self.avg_pool(x_trans).view(b, d)
...
class Backbone_VSSM(VSSM):
    ...
    def forward(self, x):
        ...
        return outs
class SaliencyMambaVision_SS2D(nn.Module):
    '''
    Local Mamba Selective Scan 2D
    '''
    def __init__(
    ...
    def forward(self, x: torch.Tensor, gt: torch.Tensor):
        ...
        if self.d_conv > 1:
            ...
            x_trans = x.permute(0, 3, 1, 2).contiguous()
            x_trans_1d=rearrange(x_trans,'b c w h -> b c (w h)')
            x_conv = F.silu(F.conv1d(input=x_trans_1d, weight=self.conv1d.weight, bias=self.conv1d.bias, padding='same', groups=self.d_inner))
            x_conv=rearrange(x_conv,'b c (w h) -> b c w h',w=w,h=h)
