...
def randomCrop(image, label, forward, gray):
    border = 30
...
class Dataset(data.Dataset):
    def __init__(self, root_dir, mode='train', transform=None, return_size=True,gray=False):
        """
        ...
        gt_dir  = os.path.join('/mnt/d/Camouflage/',root_dir, mode, 'pseudo_mask')
        scribble_dir=os.path.join('/mnt/d/Camouflage/',root_dir, mode, 'scribble')
    ...
    def __getitem__(self, idx):
        ...
        gray_im = Image.open(meta['img_path']).convert('L')
        ...
        if self.mode == 'train':
            image, label, scribble_np, gray = randomCrop(image, label, scribble_np,gray_im)
        forward = (scribble_np == 1).astype(np.float32)
        
        masks = (scribble_np == 1) | (scribble_np == 0)  # True/False
        masks = masks.astype(np.float32)                 # 0.0 / 1.0
