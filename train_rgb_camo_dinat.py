...
parser.add_argument('--save_fold', type=str, default='./checkpoints_camo_Dinat_nothing')  # 训练过程中输出的保存路径
...
config.save_fold = config.save_fold + '/' + 'rgb_datasets'
    ...
    with torch.no_grad():
        for i, data_batch in enumerate(test_loader):
            ...
            if y == 0:
                ...
                Q = alpha * S_object(pre1, gt) + (1 - alpha) * S_region(pre1, gt,0)
                if Q.item() < 0:
...
if __name__ == '__main__':
    ...
    for epoch in range(config.epoch):
        ...
        for i, data_batch in enumerate(dataloader):
            image, label,foward,masks,gray= data_batch['image'], data_batch['label'],data_batch['forward'],data_batch['masks'],data_batch['gray']
            if image.size()[2:] != label.size()[2:]:
                print("Skip this batch")
                continue
            if config.cuda:
                image, label,foward,masks,gray= image.to(device), label.to(device),foward.to(device),masks.to(device),gray.to(device)
            
            img_size = image.size(2) * image.size(3) * image.size(0)
            ratio = img_size / torch.sum(masks)
            out, saliency = model(image)
            # sig_saliency = torch.sigmoid(saliency)*masks
