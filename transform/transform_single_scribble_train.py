...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        if self.padding > 0:
            ...
            if gray is not None:
                gray = ImageOps.expand(Image.fromarray(gray), border=self.padding, fill=0)
                gray = np.array(gray)
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        if 'gray' in sample:
            output['gray'] = sample['gray'].astype(np.float32)
...
class ToTensor(object):
    def __call__(self, sample):
        ...
        if 'gray' in sample:
            gray = np.expand_dims(sample['gray'], axis=0)
...
class FixedResize(object):
    ...
    def __call__(self, sample):
        ...
        if gray is not None:
            gray = cv2.resize(gray, self.size, interpolation=cv2.INTER_NEAREST)
    ...
    def __call__(self, sample):
        ...
        if random.random() > 0.8:
            ...
            if forward is not None:
                ...
                forward = np.array(forward)
            if gray is not None:
                gray = Image.fromarray(gray).rotate(angle, Image.NEAREST)
                gray = np.array(gray)
