...
def make_min(gt_x):#개쓰래기임으로 폐기 
   
    a=torch.zeros_like(gt_x)
    
    mean_value=torch.mean(gt_x)
    min_value = torch.min(gt_x)
    a[gt_x < mean_value] = min_value
