import argparse
import os
from dataset.rgb_dataset_polyp_test import Dataset
import torch
from torchvision import transforms
from transform import transform_single
from torch.utils import data
from models.SalMamba_single_Dinat import Model
import numpy as np
import cv2
...
parser.add_argument('--model_path', type=str, default='./checkpoints_polyp/rgb_datasets/epoch_16_best.pth')#
parser.add_argument('--test_dataset', type=str, default='ETIS-LaribPolypDB')
parser.add_argument('--testsavefold', type=str, default='./ETIS-LaribPolypDB')
#CVC-ClinicDB. CVC-300,Kvasir,CVC-ColonDB,ETIS-LaribPolypDB
# Misc
...
dataset = Dataset(root_dir=config.test_dataset, transform=composed_transforms_te, mode='test')
...
for i, data_batch in enumerate(test_loader):
    ...
    with torch.no_grad():
        ...
        for i in range(config.test_batch_size):
            presavefold = os.path.join(config.testsavefold)
            ...
            name[i] = os.path.splitext(name[i])[0]+'.png'
